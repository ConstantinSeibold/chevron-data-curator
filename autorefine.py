...
def classify_shape(mask: np.ndarray) -> str:
    """`line` if the mask is THIN (median tube width « extent), else `blob`. Width-based, so it is
    robust to holes (an annulus' long skeleton would fool an elongation ratio, but its width is large)."""
    from scipy import ndimage as ndi
    m = mask > 0
    if m.sum() < 8:
        return "blob"
    ...
    return "line" if width / (_bbox_diag(m) + 1e-9) < _THINNESS_LINE else "blob"
...
def search(gray: np.ndarray, mask: np.ndarray, *, kind: str = "auto",
           candidates: list[list[dict]] | None = None) -> dict:
    """Score every candidate chain on this mask, return the ranked list + the argmax `best`.

    Returns {kind, best: {chain, score, breakdown, ncc, area}, candidates: [... sorted desc]}.
    `best.chain` is `[]` (no-op) when the mask is already clean."""
    from . import refine

    m = mask > 0
    if m.sum() < 4:
        return {"kind": "blob", "best": {"chain": [], "score": 0.0, "breakdown": {"empty": True},
                                         "ncc": 0, "area": int(m.sum())}, "candidates": []}
    ...
    return {"kind": kind, "best": best, "candidates": results}
